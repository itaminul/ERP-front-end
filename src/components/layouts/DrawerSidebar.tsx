...
const DrawerSideBar = () => {
   ...
   return (
      <>
      <div className="flex">
         <div className={`${open ? "w-48": 'w-30'} duration-300 h-screen pt-0 bg-sky-500 relative`}>

            <img src="arrow-icon.webp"
            className={`absolute cursor-pointer -right-3  top-2 w-10 border-2 border-dark-purple rounded-full ${!open && 'rotate-180'}`}
            onClick={() => setOpen(!open)}
            />
            <div>
            <div className={`py-4 overflow-y-auto ${!open && "scale-0"}`}>
